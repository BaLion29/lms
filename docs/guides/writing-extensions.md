...
```json
{
  ...
  "version": "0.1.0",
  "depends_on": [
    {"name": "address_book", "range": ">=0.1.0 <0.2.0"},
...
`address_book`, version 0.1.0) that contributes the `address_book` schema
...
└── src/firnline_ext_address_book/
    ...
    ├── manifest.json     # name: "address_book", version: "0.1.0"
...
Add to `.env`:
```
FIRNLINE_EXTENSIONS=...,firnline_ext_address_book-0.1.0-py3-none-any.whl
```
