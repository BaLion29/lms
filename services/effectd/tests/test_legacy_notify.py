"""Tests for effectd.legacy_notify — no network, AsyncMock TdbClient, fake channel objects."""
...
def _make_loop(
    *,
    ...
    """Build a LegacyNotifyLoop backed by an AsyncMock TdbClient wrapped in FakeRepo."""
    tdb = AsyncMock()
    ...
    return LegacyNotifyLoop(repo=repo, channels=channels, now=lambda: now)
...
class TestPendingToNotified:
    ...
    async def test_pending_firing_notified_with_count_1(self):
        ...
        loop.repo.transition.assert_called_once_with(
            "TriggerFiring/f1",
            "status",
            "pending",
            "notified",
        ...
        loop.repo.tdb.insert_documents.assert_called_once()
        bump_call = loop.repo.tdb.insert_documents.call_args[0]
    ...
    async def test_pending_with_subject_resolved(self):
        ...
        loop.repo.transition.assert_called_once()
    ...
    async def test_all_channels_fail_leaves_pending(self):
        ...
        assert len(channel.calls) == 1
        loop.repo.transition.assert_not_called()
        loop.repo.tdb.insert_documents.assert_not_called()
...
class TestNoChannelsIdle:
    ...
    async def test_no_channels_idles_gracefully(self):
        ...
        loop = _make_loop(channels=[])

        await loop.run_cycle()

        loop.repo.tdb.get_documents_by_status.assert_not_called()
        loop.repo.tdb.insert_documents.assert_not_called()
...
class TestRenotifyAndExpiry:
    ...
    async def test_renotify_fires_after_renotify_every(self):
        ...
        assert len(channel.calls) == 1
        loop.repo.tdb.insert_documents.assert_called_once()
        updated_doc = loop.repo.tdb.insert_documents.call_args[0][0][0]
...
class TestSnoozed:
    ...
    async def test_snoozed_wakes_up_and_notifies(self):
        ...
        loop.repo.tdb.insert_documents.assert_called_once()
        bump_docs = loop.repo.tdb.insert_documents.call_args[0][0]
        assert len(bump_docs) == 1
