"""Integration tests for firnline-schema apply/validate/promote against a dev instance.
...
async def _run_cli(args: list[str]) -> tuple[int, str, str]:
    """Run firnline-schema CLI with args and return (exit_code, stdout, stderr)."""
    import subprocess
    import sys

    cmd = [sys.executable, "-m", "firnline_schema.cli"] + args
