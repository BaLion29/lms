...
from firnline_schema.discovery import (
    DiscoveryError,
    ModuleSource,
    discover_module_dirs,
)
from firnline_schema import SchemaError
    ...
    def test_successful_discovery(self, tmp_path: Path) -> None:
        ...
        eps = [
            FakeEntryPoint(
                ...
                dist_name="firnline-ext-test",
                dist_version="2.0.0",
            ),
        ]
        with patch("firnline_schema.discovery.importlib.metadata.entry_points", return_value=eps):
            result = discover_module_dirs()
        assert "ext1" in result
        ms = result["ext1"]
        assert ms.name == "ext1"
        assert ms.path == mod_dir
        assert ms.origin == "pkg:firnline-ext-test==2.0.0"
...
class TestComposeWithEntryPoints:
    ...
    def test_pkg_only_module_composes(self, tmp_path: Path) -> None:
        ...
        ep_source = ModuleSource(
            name="extmod",
            path=pkg_dir / "extmod",
            origin="pkg:firnline-ext-test==2.0.0",
        )
        ...
        assert ext_info.source == "pkg:firnline-ext-test==2.0.0"
    ...
    def test_duplicate_name_across_sources(self, tmp_path: Path) -> None:
        ...
        ep_source = ModuleSource(
            name="dupe",
            path=pkg_dir / "dupe",
            origin="pkg:firnline-ext-dupe==1.0.0",
        )
        ...
        assert "pkg:firnline-ext-dupe==1.0.0" in msg
    ...
    def test_merged_topo_and_l2_works(self, tmp_path: Path) -> None:
        ...
        ep_source = ModuleSource(name="extmod", path=pkg_dir / "extmod", origin="pkg:firnline-ext-test==2.0.0")
        result = compose(repo, entry_point_modules={"extmod": ep_source})
        names = [m.name for m in result.modules]
        assert names[0] == "core"
...
class TestLockSourceRecording:
    ...
    def test_pkg_module_has_pkg_source(self, tmp_path: Path) -> None:
        ...
        ep_source = ModuleSource(name="extmod", path=pkg_dir / "extmod", origin="pkg:firnline-ext-test==3.2.1")
        result = compose(repo, entry_point_modules={"extmod": ep_source})
        ext_info = next(m for m in result.modules if m.name == "extmod")
        assert ext_info.source == "pkg:firnline-ext-test==3.2.1"
    ...
    def test_source_present_in_compose_result(self, tmp_path: Path) -> None:
        ...
        ep_source = ModuleSource(name="pkg_mod", path=pkg_dir / "pkg_mod", origin="pkg:firnline-ext-pkg==1.0.0")
        ...
        assert sources["pkg_mod"] == "pkg:firnline-ext-pkg==1.0.0"
...
class TestComposeCliNoEntryPoints:
    ...
    def test_flag_skips_entry_points(self, tmp_path: Path) -> None:
        ...
        ep_source = ModuleSource(name="extmod", path=pkg_dir / "extmod", origin="pkg:firnline-ext==1.0.0")
...
class TestDiscoveryEdgeCases:
    ...
    def test_no_entry_points_registered(self) -> None:
        """No firnline.schema_modules entry points → empty result, no error."""
        with patch("firnline_schema.discovery.importlib.metadata.entry_points", return_value=[]):
            result = discover_module_dirs()
        assert result == {}
