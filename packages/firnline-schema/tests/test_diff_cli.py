...
def _run_diff(modules_dir: Path, baseline_modules: Path | None = None, baseline_lock: Path | None = None) -> subprocess.CompletedProcess:
    cmd = [sys.executable, "-m", "firnline_schema.cli", "diff", "--modules-dir", str(modules_dir), "--no-entry-points"]
    if baseline_modules:
...
class TestDiffExitCodes:
    ...
    def test_plan_missing_tdb_args_exit_2(self, tmp_path: Path):
        ...
        _make_core(mod_dir)
        cmd = [sys.executable, "-m", "firnline_schema.cli", "plan", "--modules-dir", str(mod_dir)]
    ...
    def test_plan_partial_tdb_args_exit_2(self, tmp_path: Path):
        ...
        mod_dir = tmp_path / "mods"
        _make_core(mod_dir)
        cmd = [sys.executable, "-m", "firnline_schema.cli", "plan",
    ...
    def test_apply_missing_tdb_args_exit_2(self, tmp_path: Path):
        ...
        mod_dir = tmp_path / "mods"
        _make_core(mod_dir)
        cmd = [sys.executable, "-m", "firnline_schema.cli", "apply", "--modules-dir", str(mod_dir)]
        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 2, result.stderr
    ...
    def test_plan_env_fallback(self, tmp_path: Path, monkeypatch):
        ...
        mod_dir = tmp_path / "mods"
        _make_core(mod_dir)
        monkeypatch.setenv("FIRNLINE_SCHEMA_TDB_PASSWORD", "test")
        cmd = [sys.executable, "-m", "firnline_schema.cli", "plan",
               "--modules-dir", str(mod_dir),
    ...
    def test_no_changes_exit_0(self, tmp_path: Path):
        ...
        from firnline_schema.composer import compose
    ...
    def test_tdb_partial_args_exit_2(self, tmp_path: Path):
        ...
        cmd = [
            sys.executable, "-m", "firnline_schema.cli", "diff",
            "--modules-dir", str(mod_dir),
            "--tdb-url", "http://localhost:6363",
        ]
...
def test_diff_classifies_entry_point_module_changes(tmp_path: Path) -> None:
    ...
    from firnline_schema.cli import _diff_fragments
    ...
    from firnline_schema.composer import fragment_checksum
    ...
    fake_ms = ModuleSource(name="extmod", path=pkg_dir, origin="pkg:firnline-ext-test==1.1.0")
    with patch("firnline_schema.discovery.discover_module_dirs", return_value={"extmod": fake_ms}):
