"""firnline-schema CLI — schema module composition and lifecycle management.
...
def _cmd_codegen(args: argparse.Namespace) -> int:
    ...
    if not composed_path.is_file():
        print(
            ...
            f"  Run 'firnline-schema compose' first to generate it.",
            file=sys.stderr,
        )
        return 1
...
def _cmd_diff(args: argparse.Namespace) -> int:
    ...
    tdb_password = args.tdb_password or os.environ.get("FIRNLINE_SCHEMA_TDB_PASSWORD", "")
    ...
    if tdb_args_provided:
        ...
        if not tdb_password:
            missing.append("--tdb-password (or FIRNLINE_SCHEMA_TDB_PASSWORD env var)")
        if missing:
            print(
    ...
    if args.tdb_url:
        ...
        try:
            ...
            from firnline_core.tdb import TdbError  # delayed import
...
def _validate_tdb_args(args: argparse.Namespace) -> str:
    ...
    if not tdb_args_provided:
        print(
            ...
            "--tdb-db, --tdb-user, and --tdb-password (or FIRNLINE_SCHEMA_TDB_PASSWORD env var).",
            file=sys.stderr,
        )
        sys.exit(2)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="firnline-schema",
        description="Firnline Schema Module System CLI",
    )
    ...
    p_compose.add_argument(
        ...
        action="store_true",
        default=False,
        help="Skip discovery of installed firnline.schema_modules entry points",
    )
    ...
    p_diff.add_argument(
        "--tdb-password",
        default=None,
        help="TerminusDB password (falls back to FIRNLINE_SCHEMA_TDB_PASSWORD env var)",
    )
