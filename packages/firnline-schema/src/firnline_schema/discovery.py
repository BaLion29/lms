"""Schema module discovery via ``firnline.schema_modules`` entry points.

Each entry point in the ``firnline.schema_modules`` group points to a directory
...
def discover_module_dirs() -> dict[str, ModuleSource]:
    """Discover schema modules from installed ``firnline.schema_modules`` entry points.
    ...
    eps = importlib.metadata.entry_points(group="firnline.schema_modules")
    for ep in eps:
