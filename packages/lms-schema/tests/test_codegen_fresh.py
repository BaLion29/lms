...
committed files in ``packages/lms-core/src/lms_core/generated/``.
"""
...
from lms_schema.codegen import schema_checksum, write_generated

MODULES_DIR = Path(__file__).parents[3] / "schema" / "modules"
GENERATED_DIR = Path(__file__).parents[3] / "packages" / "lms-core" / "src" / "lms_core" / "generated"
...
def test_codegen_freshness(tmp_path: Path):
    ...
    for cf in committed_files:
        ...
        assert cf.read_bytes() == tf.read_bytes(), (
            ...
            f"Run 'lms-schema compose && lms-schema codegen' and commit the result."
        )

    # Also verify the generated package imports cleanly
    import lms_core.generated  # noqa: F401 — ensures __init__.py works
    assert lms_core.generated.InboxNote is not None
