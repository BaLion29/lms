...
def _enum_values(cls: dict[str, Any]) -> set[str]:
    """Return the set of @values for an enum class."""
    vals = cls.get("@value")
    ...
    for key in old_props & new_props:
        ...
        if _canonical(old_val) != _canonical(new_val):
            changes.append(Change(
                module="",
                kind="breaking",
                description=f"Class '{cid}': property '{key}' range/type changed",
            ))
...
def diff_against_live(
    ...
    for cid in sorted(fetched_ids - current_ids):
        changes.append(Change(module=default_module, kind="breaking", description=f"Class '{cid}' removed (in live instance but not composed)"))
...
def check_guardrails(
    ...
    if new_version == old_version and changes:
        violations.append(
            f"Module '{module_name}': fragment checksum changed but version "
