"""Async HTTP clients for UI service communication — framework-free.

Shared by the WebUI and the TUI.  Tokens are kept server-side — never
exposed to the browser or terminal.  All clients raise
:class:`UiClientError` on failure.
"""
...
class UiClientError(Exception):
    """Raised by UI clients on HTTP/non-JSON errors."""

    def __init__(self, status: int | None, detail: str) -> None:
        self.status = status
        self.detail = detail
        super().__init__(f"UiClientError({status}): {detail}")
...
async def _healthz_raw(client: httpx.AsyncClient, path: str = "/healthz") -> dict[str, Any]:
    ...
    try:
        ...
        raise UiClientError(None, f"transport error: {exc!s}") from exc
    try:
        data: dict[str, Any] = resp.json()
    except ValueError as exc:
        raise UiClientError(resp.status_code, f"non-JSON response: {resp.text[:500]}") from exc
    return data
...
class TdbBrowser:
    """Thin wrapper around ``firnline_core.tdb.TdbClient`` for UI services.

    Translates ``TdbError`` to ``UiClientError`` and exposes a subset of
    ...
    async def _call(self, coro):
        try:
            ...
            raise UiClientError(exc.status, exc.body) from exc
